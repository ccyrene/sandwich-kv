"""
Iter 38: Llama-family multi-model benchmark (with HF token).

Models to test (head_dim = 128):
1. R1-Distill-Llama-8B (canonical, already known to work)
2. meta-llama/Llama-3.1-8B-Instruct (newly accessible)
3. meta-llama/Llama-3.2-3B-Instruct (newly accessible)
4. NousResearch/Meta-Llama-3-8B-Instruct (mirror, fallback if gated still fails)
5. NousResearch/Llama-2-7b-chat-hf (older Llama-2 reference)
6. mistralai/Mistral-7B-Instruct-v0.3 (Llama-style architecture for comparison)

For each: full Pareto + per-layer sensitivity profile + decision rule.
"""
...
OUT = Path("/workspace/iter38_llama_family.json")
N_CHUNKS = 6
CHUNK_LEN = 1024
...
def profile_sensitivity(model, tok, chunks, head_dim, n_layers, ppl_fp16, sample_layers=None):
    ...
    sensitivity = {}
    layers_to_test = sample_layers if sample_layers is not None else range(n_layers)
...
def benchmark_one(model_id, name):
    ...
    try:
        ...
        print(f"  ⚠️ Load failed: {str(e)[:300]}")
    ...
    print(f"  n_layers={n_layers}, head_dim={head_dim}, n_kv={n_kv_heads}, n_q={n_q_heads}", flush=True)
    ...
    print(f"  PPL = {ppl_fp16:.4f}", flush=True)
    if ppl_fp16 > 1000:
    ...
    print(f"\n{'config':<25} {'PPL':<10} {'Δppl':<10}", flush=True)
    print("-" * 50)
    for name_, k_fn, v_fn, kb, vb in configs:
        if k_fn is None and v_fn is None:
            print(f"{name_:<25} {ppl_fp16:<10.4f} {0.0:<10.4f}", flush=True)
        ...
        try:
            ppl, _ = compute_perplexity(model, tok, chunks)
            delta = ppl - ppl_fp16
            print(f"{name_:<25} {ppl:<10.4f} {delta:+.4f}", flush=True)
            results[name_] = {"K_bits": kb, "V_bits": vb, "ppl": float(ppl), "delta_ppl": float(delta)}
        except Exception as e:
            print(f"{name_:<25}: ERROR {e}", flush=True)
            results[name_] = {"error": str(e)}
        finally:
            h.remove()

    # Sensitivity profile (sample every 2 layers for speed; first 8 + last 8 fully)
    print("\n[Per-layer sensitivity profile]", flush=True)
    sample_layers = list(range(0, min(8, n_layers))) + \
                    list(range(8, n_layers - 8, 2)) + \
                    list(range(max(0, n_layers - 8), n_layers))
    sample_layers = sorted(set(sample_layers))
    sensitivity, ppl_b2 = profile_sensitivity(model, tok, chunks, head_dim, n_layers, ppl_fp16, sample_layers)
    ...
    middle_layers_in_sample = [L for L in sample_layers if n_layers // 4 <= L < 3 * n_layers // 4]
    middle_lift = np.mean([sensitivity.get(L, 0) for L in middle_layers_in_sample]) if middle_layers_in_sample else 0
    ratio = boundary_lift / max(middle_lift, 0.01)
    print(f"  Top-5 sensitive: {top5}", flush=True)
    print(f"  Boundary lift: {boundary_lift:.3f}, middle mean lift: {middle_lift:.3f}", flush=True)
    print(f"  → L0/L_middle ratio = {ratio:.2f} {'(boundary-dominant)' if ratio > 5 else '(flat)'}", flush=True)
    results["sensitivity_profile"] = {
        ...
        "top5_sensitive_layers": [(int(L), float(v)) for L, v in top5],
...
def main():
    ...
    # Summary
    print(f"\n{'='*130}\nLLAMA-FAMILY CROSS-MODEL SUMMARY\n{'='*130}", flush=True)
    print(f"{'Method':<25}", end="")
    for _, name in models:
        if name in all_results and "error" not in all_results[name]:
            print(f"{name[:18]:<20}", end="")
    print()
    rows = ["FP16_PPL", "TQ_K=V=b3", "TQ_K=V=b4", "Sand_K=V_3.0",
            "Sand+RVQ_K=V_3.5", "Sand+RVQ_K=V_4.0", "ratio"]
    for cfg in rows:
        if cfg == "FP16_PPL":
            ...
            for _, name in models:
                ...
                if "error" in r:
                    ...
                    rt = sp.get("ratio", 0)
                    verdict = sp.get("verdict", "—")[:9]
                    print(f"{rt:<10.1f} {verdict:<8}", end="")
        else:
            print(f"{cfg:<25}", end="")
            for _, name in models:
                r = all_results.get(name, {})
